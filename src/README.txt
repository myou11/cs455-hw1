...
-----------------------
    Because of using a ConcurrentLinkedQueue, retrieving the traffic summaries had to be delayed for 30 seconds
    instead of 15 for large amounts of packets.

    My program does not guarantee correct functionality if registration/deregistration happens after setup-overlay
    has occured. Multiple 'start number-of-messages' can be run without having to restart the program, but if
    additional registrations occur in between start commands, the outcome of the program will likely crash. This
    is because these registrations/deregistrations are occuring after the overlay has already been setup.
